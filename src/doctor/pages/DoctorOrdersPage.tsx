...
import { ClipboardList, Video, Stethoscope, Clock, Search, User, ChevronRight } from "lucide-react";
...
export const mockConsultOrders: DoctorConsultOrder[] = [
  ...
  { id: "oc3", orderNo: "OC20260308001", patient: { name: "Peter L.", phone: "+852 6555 6666", gender: { en: "Male", zh: "男" } }, consultationType: "text_image", symptoms: { en: "Sensitivity to cold drinks on upper left teeth", zh: "上左牙齒對冷飲敏感" }, medicalHistory: { en: "None", zh: "無" }, imageCount: 1, status: "completed", amount: 200, createdAt: "2026-03-08T09:00:00", consultationStartedAt: "2026-03-08T09:20:00", consultationEndedAt: "2026-03-08T09:50:00", diagnosisNotes: { en: "Likely early-stage cavity on upper left premolar (#24). Recommend in-clinic examination and X-ray to confirm. May require composite filling.", zh: "上左前磨牙（#24）疑似早期蛀牙。建議到診檢查及X光確認。可能需要複合樹脂補牙。" }, medicationAdvice: { en: "Use desensitizing toothpaste (e.g. Sensodyne) twice daily. Avoid very cold/hot beverages. Return for in-clinic check within 2 weeks.", zh: "每天使用抗敏牙膏（如舒適達）兩次。避免過冷過熱飲品。兩週內回診檢查。" } },
  { id: "oc4", orderNo: "OC20260307001", patient: { name: "Cindy W.", phone: "+852 6777 8888", gender: { en: "Female", zh: "女" } }, consultationType: "video", symptoms: { en: "Jaw clicking and pain when opening mouth wide", zh: "大幅張嘴時顎骨彈響及疼痛" }, medicalHistory: { en: "History of bruxism", zh: "有磨牙史" }, imageCount: 0, status: "completed", amount: 380, createdAt: "2026-03-07T14:00:00", consultationStartedAt: "2026-03-07T14:10:00", consultationEndedAt: "2026-03-07T14:40:00", diagnosisNotes: { en: "Temporomandibular joint dysfunction (TMD). Likely related to bruxism history. Recommend in-clinic assessment for night guard fitting.", zh: "顳顎關節紊亂（TMD）。可能與磨牙史相關。建議到診評估及訂製夜間咬合板。" }, medicationAdvice: { en: "Apply warm compress to jaw area. Avoid hard foods. Consider ibuprofen for acute pain. Scheduled for in-clinic follow-up.", zh: "對顎部進行熱敷。避免硬食。急性疼痛可考慮布洛芬。已安排覆診。" } },
  { id: "oc5", orderNo: "OC20260306001", patient: { name: "Alex M.", phone: "+852 6999 0000", gender: { en: "Male", zh: "男" } }, consultationType: "text_image", symptoms: { en: "White spot on tongue, mild discomfort", zh: "舌頭白斑，輕微不適" }, medicalHistory: { en: "Smoker", zh: "吸煙者" }, imageCount: 3, status: "rejected", amount: 200, createdAt: "2026-03-06T16:00:00", rejectionReason: "This condition requires an oral medicine specialist. Please consult with Dr. Liu Yating who specializes in oral pathology." },
];
...
const DoctorOrdersPage: React.FC = () => {
  ...
  const clinicStatusLabel = (s: ClinicOrderStatus) => {
    const map: Record<ClinicOrderStatus, { label: string; cls: string }> = {
      pending_acceptance: { label: isEn ? "Pending" : "待接受", cls: "bg-warning/10 text-warning border-warning/20" },
      pending_treatment: { label: isEn ? "Confirmed" : "待治療", cls: "bg-primary/10 text-primary border-primary/20" },
      completed: { label: isEn ? "Completed" : "已完成", cls: "bg-success/10 text-success border-success/20" },
    };
    return map[s];
  };

  const consultStatusLabel = (s: ConsultOrderStatus) => {
    const map: Record<ConsultOrderStatus, { label: string; cls: string }> = {
      pending_acceptance: { label: isEn ? "Pending" : "待接受", cls: "bg-warning/10 text-warning border-warning/20" },
      in_consultation: { label: isEn ? "In Progress" : "諮詢中", cls: "bg-info/10 text-info border-info/20" },
      completed: { label: isEn ? "Completed" : "已完成", cls: "bg-success/10 text-success border-success/20" },
  ...
  const filteredClinic = mockClinicOrders.filter((o) => {
    ...
    if (searchQuery && !o.patient.name.toLowerCase().includes(searchQuery.toLowerCase()) && !o.orderNo.toLowerCase().includes(searchQuery.toLowerCase()) && !o.service[lang].toLowerCase().includes(searchQuery.toLowerCase())) return false;
    return true;
  });
      ...
      <Tabs value={mainTab} onValueChange={setMainTab}>
        ...
        <TabsContent value="in_clinic">
          ...
          {filteredClinic.length === 0 ? (
            <div className="py-16 text-center"><ClipboardList className="mx-auto h-12 w-12 text-muted-foreground/40" /><p className="mt-3 text-sm text-muted-foreground">{isEn ? "No orders found" : "未找到訂單"}</p></div>
          ) : (
            <div className="space-y-3">
              {filteredClinic.map((order) => {
                const st = clinicStatusLabel(order.status);
                return (
          ...
          {filteredConsult.length === 0 ? (
            ...
            <div className="space-y-3">
              {filteredConsult.map((order) => {
                const st = consultStatusLabel(order.status);
                return (
